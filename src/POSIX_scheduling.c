...
static void* Thread(void *inArgs)
{
	...
	int tid = args->thread_number, i; /* tid is just the number of the executing thread; Note that, 
	pthread_t specified thread id is no the same as this thread id as this depicts only the sequence number of this thread.*/
	struct timespec next, now, response;
	...
	while (1) { 
		...
		// abort thrread in case of deadline miss
		if (timespec_cmp(&now, &next) > 0) {
			trace_write( "Deadline miss for thread %d\n", tid); 
			trace_write("now: %d sec %ld nsec     next: %d sec %ldnsec \n", now.tv_sec, now.tv_nsec, next.tv_sec, next.tv_nsec);
			exit(-1); 
		}


	
		for (i = 0; i < (args->thread_exectime / 5000) ; j++) // run for job exec time
			workload(args->thread_number); // This produces a busy wait loop of ~5+/-100us milliseconds

		results[tid].thread_deadline 		= next.tv_nsec / 1000; // implicit deadline (next release) - microseconds

		clock_gettime(CLOCK_REALTIME, &response);
		
		

		// in each iteration, set response if larger than previous
		if((clock_diff(now, response) / 1.0e3) > results[tid].thread_response_time)
			results[tid].thread_response_time 	= response.tv_nsec / 1000;

		// this sets our period for sleeping until next iteration of the current thread 
...
int main(int argc, char **argv)
{
	...
	int priorities[NUM_THREADS] = {3,2,1,1}; /* Priorities are in the range from 1 to 99 when it comes to SCHED_FIFO and SCHED_RR, and for
	SCHED_OTHER, the priority is always 0 */

	// setting 50ms, 50 ms and 100ms, 100ms
	int periods[NUM_THREADS] = { 50000, 50000, 100000, 100000 };	// Used in calculation of next period value in a periodic task / thread.

	int exec_times[NUM_THREADS] = { 10, 20, 20, 10 }, i;
    
	struct thread_args lvargs[NUM_THREADS];

	// setting tasks exect times
	for (i = 0; i < NUM_THREADS; i++) {
		lvargs[i].thread_exectime = exec_times[i];
		lvargs[i].thread_start_time = priorities[i] *1000; // up to discussion, currently delaying higher prios 
	}
